...
@export var wall_jump_buffer_time: float
@export var wall_jump_coyote_time: float

@onready var jump_buffer_timer = TimerUtil.create_basic_timer(self, jump_buffer_time)
@onready var jump_coyote_time_timer = TimerUtil.create_basic_timer(self, jump_coyote_time)
@onready var wall_jump_buffer_timer = TimerUtil.create_basic_timer(self, wall_jump_buffer_time)
@onready var wall_jump_coyote_time_timer = TimerUtil.create_basic_timer(self, wall_jump_coyote_time)
...
func handle_jump(body: CharacterBody2D, wants_to_jump: bool, released_jump: bool):
	...
	_handle_wall_jump_coyote_time(body)
	_handle_wall_jump_buffer(body, wants_to_jump)
...
func _is_allowed_to_jump(body: CharacterBody2D, wants_to_jump: bool) -> bool:
	return wants_to_jump and (body.is_on_floor() or not jump_coyote_time_timer.is_stopped())
...
func _handle_jump_coyote_time(body: CharacterBody2D):
	if _has_just_stepped_off_ledge(body):
		jump_coyote_time_timer.start()
...
func _is_allowed_to_wall_jump(body: CharacterBody2D, wants_to_jump: bool) -> bool:
	return wants_to_jump and (body.is_on_wall_only() or not wall_jump_coyote_time_timer.is_stopped())
...
func _handle_wall_jump_coyote_time(body: CharacterBody2D):
	if _has_just_left_wall(body):
		wall_jump_coyote_time_timer.start()
...
func _handle_wall_jump_buffer(body: CharacterBody2D, wants_to_jump: bool):
	if wants_to_jump and not body.is_on_wall() and not was_on_floor_last_frame:
		wall_jump_buffer_timer.start()
	
	if body.is_on_wall_only() and not wall_jump_buffer_timer.is_stopped():
		_execute_wall_jump(body)


func _stop_all_timers():
	jump_buffer_timer.stop()
	jump_coyote_time_timer.stop()
	wall_jump_coyote_time_timer.stop()
