...
func _calculate_wind_gust(direction: Vector2):
	...
	var calculated_polygon = PolygonUtil.get_global_polygon_from_local_space(rotated_polygon, player.get_center_global_position())
	...
	PhysicsManager.ImpulseBuilder.new()\
		...
		.applied_body_impulse(_get_body_push_force.bindv([player, direction]))\
		.applied_player_impulse(_get_player_push_force.bindv([player, direction]))\
		.excluded_players_from_impulse([player])\
		.execute()
...
func _display_wind_gust_texture(direction: Vector2):
	...
	sprite.texture = preload("res://spells/textures/shitty_wind_gust_texture.png")
